import { x as createScope, n as signal, e as effect, t as isString, y as isObject, a as isNumber, z as isBoolean } from '../chunks/vidstack-DoOmgEfT.js';
import '../chunks/vidstack-awVFxJvn.js';
import { T as TimeRange } from '../chunks/vidstack-BYuWkQD6.js';
import { p as preconnect } from '../chunks/vidstack-CMz4Bd8X.js';
import { E as EmbedProvider, t as timedPromise } from '../chunks/vidstack-DPZGEOYG.js';
...
class YouTubeProvider extends EmbedProvider {
  ...
  Eb(time, trigger) {
    const { duration, realCurrentTime } = this.b.$state, hasEnded = this.mc === YouTubePlayerState.Ue, boundTime = hasEnded ? duration() : time, detail = {
      currentTime: boundTime,
      played: this.cm(boundTime)
    };
  ...
  Rg(state, trigger) {
    const { started, paused, seeking } = this.b.$state, isPlaying = state === YouTubePlayerState.Ve, isBuffering = state === YouTubePlayerState.Ng, isPlay = (paused() || this.E) && (isBuffering || isPlaying);
    if (isBuffering)
      this.c("waiting", void 0, trigger);
    if (seeking() && isPlaying) {
      this.bb(trigger);
    }
    if (!started() && isPlay && this.Jk) {
      this.E?.reject("invalid internal play operation");
      this.E = null;
      if (isPlaying) {
        this.pause();
        this.Jk = false;
