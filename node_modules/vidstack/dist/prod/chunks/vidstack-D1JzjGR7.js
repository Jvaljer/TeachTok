import { a1 as ViewController, l as listenEvent, o as onDispose, n as signal, u as peek, D as DOMEvent, C as Component, e as effect, K as isKeyboardEvent, r as isArray, t as isString, c as isKeyboardClick, a2 as waitIdlePeriod, O as untrack, T as tick, A as deferredPromise, i as isUndefined, a as isNumber, s as setAttribute, q as computed, W as provideContext, p as animationFrameThrottle, _ as uppercaseFirstChar, L as camelToKebabCase, j as setStyle, Y as prop, a3 as method, h as scoped, Z as noop, S as State, V as createContext, v as useContext, a4 as ariaBool$1, a5 as isWriteSignal, U as isNull, a6 as hasProvidedContext, y as isObject, B as useState, x as createScope, J as isPointerEvent, f as isTouchEvent, a7 as isMouseEvent, a8 as kebabToCamelCase, X as createDisposalBin } from './vidstack-DoOmgEfT.js';
import { L as List, s as softResetMediaState, m as mediaState } from './vidstack-awVFxJvn.js';
import { u as useMediaContext, m as mediaContext } from './vidstack-i0oJKwBM.js';
import { i as isTrackCaptionKind, b as TextTrackSymbol, T as TextTrack, p as preconnect, g as getRequestCredentials, w as watchActiveTextTrack, d as isCueActive } from './vidstack-CMz4Bd8X.js';
import { i as isTouchPinchEvent, s as setAttributeIfEmpty, j as hasAnimation, r as requestScopedAnimationFrame, k as autoPlacement, o as onPress, m as setARIALabel, n as isElementParent } from './vidstack-CyIpzU-o.js';
import { L as ListSymbol } from './vidstack-CerDQCFL.js';
import { Q as QualitySymbol } from './vidstack-B3EgHpRH.js';
import { d as canOrientScreen, j as canPlayAudioType, k as canPlayVideoType, i as isHLSSupported, I as IS_CHROME, a as IS_IOS, b as canChangeVolume, l as IS_IPHONE } from './vidstack-B_9E7csz.js';
import { c as coerceToError, a as assert } from './vidstack-2fIzRyOW.js';
import { i as isAudioSrc, e as isVideoSrc, f as isHLSSrc, c as canGoogleCastSrc } from './vidstack-XO9MYVWR.js';
import { a as getTimeRangesEnd } from './vidstack-BYuWkQD6.js';
...
class TextTrackList extends List {
  constructor() {
    ...
    this.un = functionDebounce(async () => {
      ...
      const showCaptions = await this.Sk?.getCaptions(), kinds = [
        ["captions", "subtitles"],
        "chapters",
        "descriptions",
        "metadata"
  ...
  vn(track) {
    ...
    this.Sk?.setCaptions?.(track.mode === "showing");
  }
  tn(lang) {
    this.Sk?.setLang?.(this.Rk = lang);
  }
  setStorage(storage) {
    this.Sk = storage;
  }
...
class MediaKeyboardController extends MediaPlayerController {
  ...
  Ab(event) {
    ...
    let { method, value } = this.Zc(event), isNumberPress = !event.metaKey && /^[0-9]$/.test(event.key);
...
class MediaRequestManager extends MediaPlayerController {
  ...
  ll() {
    const { canAirPlay } = this.$state, supported = !!this.uk()?.airPlay?.supported;
  ...
  sh() {
    const { canFullscreen } = this.$state, supported = this.rc.supported || !!this.uk()?.fullscreen?.supported;
    canFullscreen.set(supported);
  }
  th() {
    const { canPictureInPicture } = this.$state, supported = !!this.uk()?.pictureInPicture?.supported;
  ...
  async ul(trigger) {
    try {
      ...
      if (!this.om) {
        const $module = await import('./vidstack-6eBVBXTk.js');
...
class MediaStateManager extends MediaPlayerController {
  ...
  ["remote-playback-change"](event) {
    ...
    if (isConnected) {
      ...
      const requestEvent = this.f.e.kl(key);
      if (requestEvent) {
        event.request = requestEvent;
        event.triggers.add(requestEvent);
      }
...
class NavigatorMediaSession extends MediaPlayerController {
  static {
    this.Nm = ["play", "pause", "seekforward", "seekbackward", "seekto"];
  }
  constructor() {
    super();
  }
  onConnect() {
    ...
    for (const action of NavigatorMediaSession.Nm) {
...
const _MediaPlayer = class _MediaPlayer extends Component {
  ...
  Tm() {
    ...
    if (storage?.onChange) {
      const { source } = this.$state, playerId = isString(storageValue) ? storageValue : this.el?.id, mediaId = computed(this.Um.bind(this));
...
class SliderValue extends Component {
  ...
  getValueText() {
    const { type, format, decimalPlaces, padHours, padMinutes, showHours, showMs } = this.$props, { value: sliderValue, pointerValue, min, max } = this.Q, _format = format() ?? this.Sb.default;
...
class SliderChapters extends Component {
  constructor() {
    ...
    this.Xi = animationFrameThrottle((bufferedPercent) => {
      ...
      for (let i = this.Zd; i < this.R.length; i++) {
        percent = this.ae(cues[i], bufferedPercent, startTime, endTime);
  ...
  Lf(startIndex, percent) {
    ...
    for (let i = startIndex; i < cues.length; i++) {
      chapterPercent = this.ae(cues[i], percent, startTime, endTime);
...
class Menu extends Component {
  ...
  nj(event) {
    ...
    if (!isTargetNode || this.u()?.contains(event.target))
      return;
    if (this.isSubmenu)
...
export { SliderValue as $, ARIAKeyShortcuts as A, isHTMLIFrameElement as B, sliderContext as C, Controls as D, Tooltip as E, FocusVisibleController as F, TooltipContent as G, HLSProviderLoader as H, ToggleButton as I, AirPlayButton as J, GoogleCastButton as K, LocalMediaStorage as L, MenuButton as M, CaptionButton as N, FullscreenButton as O, PlayButton as P, MuteButton as Q, PIPButton as R, Slider as S, Thumbnail as T, SeekButton as U, VideoQualityList as V, LiveButton as W, sliderState as X, YouTubeProviderLoader as Y, SliderController as Z, SliderVideo as _, MediaProvider as a, SliderPreview as a0, updateSliderPreviewPlacement as a1, VolumeSlider as a2, TimeSlider as a3, SliderChapters as a4, Menu as a5, MenuPortal as a6, menuPortalContext as a7, MenuItems as a8, RadioGroup as a9, Radio as aa, ChaptersRadioGroup as ab, AudioRadioGroup as ac, CaptionsRadioGroup as ad, DEFAULT_PLAYBACK_RATES as ae, SpeedRadioGroup as af, QualityRadioGroup as ag, Gesture as ah, Captions as ai, Poster as aj, Time as ak, ThumbnailsLoader as al, MediaPlayer as b, formatSpokenTime as c, FullscreenController as d, canFullscreen as e, formatTime as f, ScreenOrientationController as g, MediaRemoteControl as h, MediaControls as i, MEDIA_KEY_SHORTCUTS as j, TextRenderers as k, TextTrackList as l, AudioTrackList as m, AudioProviderLoader as n, VideoProviderLoader as o, VimeoProviderLoader as p, isAudioProvider as q, isVideoProvider as r, isHLSProvider as s, tooltipContext as t, isYouTubeProvider as u, isVimeoProvider as v, isGoogleCastProvider as w, isHTMLAudioElement as x, isHTMLVideoElement as y, isHTMLMediaElement as z };
