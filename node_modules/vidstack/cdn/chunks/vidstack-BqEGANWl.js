...
class VimeoProviderLoader {
  ...
  async load(ctx) {
    ...
    return new (await Promise.resolve().then(function () { return provider$1; })).VimeoProvider(this.target, ctx);
  }
  async loadPoster(src, ctx, abort) {
    const { resolveVimeoVideoId, getVimeoVideoInfo } = await Promise.resolve().then(function () { return utils; });
    if (!isString(src.src))
      return null;
...
class YouTubeProviderLoader {
  ...
  async load(ctx) {
    ...
    return new (await Promise.resolve().then(function () { return provider$2; })).YouTubeProvider(this.target, ctx);
  }
  async loadPoster(src, ctx, abort) {
    const { findYouTubePoster, resolveYouTubeVideoId } = await Promise.resolve().then(function () { return utils$1; });
...
function normalizeSrc(src) {
  return (isArray$1(src) ? src : [src && !isString(src) && "src" in src ? src : { src: src || "" }]).map(({ src: src2, type, ...props }) => ({
    src: src2,
...
class SlotObserver {
  ...
  _update(entries) {
    if (entries && !entries.some((e) => e.addedNodes.length))
      return;
    let changed = false, slots = this._root.querySelectorAll("slot");
  ...
  return $signal(() => {
    ...
    return html`
      <media-menu class="vds-font-menu vds-menu">
        ${renderMenuButton({
      label: () => i18n$1(translations, "Font Styles"),
      icon: "menu-font"
    })}
...
function DefaultFontSettingMenu({
  ...
  const { player } = useMediaContext(), { translations } = useDefaultLayoutContext(), resets = useContext(resetContext), radioOptions = createRadioOptions(options), key = `${label.toLowerCase().replace(/\s/g, "-")}`, $value = signal(defaultValue), $label = () => i18n$1(translations, label), $hint = () => {
    ...
    <media-tooltip class="vds-seek-tooltip vds-tooltip">
      <media-tooltip-trigger>
        <media-seek-button
          ...
          aria-label=${$label}
        >
          ${!backward ? html`<slot name="seek-forward-icon"></slot>` : html`<slot name="seek-backward-icon"></slot>`}
        </media-seek-button>
      </media-tooltip-trigger>
      <media-tooltip-content class="vds-tooltip-content" placement=${tooltip}>
...
function DefaultAudioMenu() {
  const { translations } = useDefaultLayoutContext(), $defaultText = $i18n$1(translations, "Default");
  return html`
...
function DefaultSpeedMenu() {
  const { translations, playbackRates } = useDefaultLayoutContext(), $normalText = $i18n$1(translations, "Normal");
  return html`
...
function DefaultQualityMenu() {
  const { translations } = useDefaultLayoutContext(), $autoText = $i18n$1(translations, "Auto");
  return html`
...
function DefaultCaptionsMenu() {
  const { translations } = useDefaultLayoutContext(), $offText = $i18n$1(translations, "Off");
  return html`
...
function getStatusLabel() {
  ...
  return $text ? i18n$1(translations, $text) : null;
}
...
function DownloadButton() {
  return $signal(() => {
    const media = useMediaContext(), { translations, download } = usePlyrLayoutContext(), { source } = media.$state, src = source(), info = download(), url = isString(info) ? info : info?.url, filename = (!isString(info) ? info?.filename : null) || "media", $downloadText = $i18n(translations, "Download");
    return url || isAudioSrc(src) || isVideoSrc(src) ? html`
          <a
            class="plyr__controls__item plyr__control"
...
const svgTemplate$1 = /* @__PURE__ */ createTemplate(
...
class MediaIconElement extends HTMLElement {
  ...
  _createSVG() {
    return cloneTemplateContent(svgTemplate$1);
  }
  _loadIcon() {
...
class VideoRemotePlaybackAdapter {
  ...
  _setup() {
    if (!this._video.remote || !this._canPrompt)
      return;
...
const videoIdRE$1 = /(?:youtu\.be|youtube|youtube\.com|youtube-nocookie\.com)\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=|)((?:\w|-){11})/;
const posterCache = /* @__PURE__ */ new Map();
const pendingFetch$1 = /* @__PURE__ */ new Map();
function resolveYouTubeVideoId(src) {
  return src.match(videoIdRE$1)?.[1];
}
async function findYouTubePoster(videoId, abort) {
  ...
  }).catch(() => "").finally(() => pendingFetch$1.delete(videoId));
  pendingFetch$1.set(videoId, pending);
  return pending;
}
...
export { AudioProvider as A, GoogleCastProvider as G, HTMLMediaProvider as H, VideoProvider as V, YouTubeProvider as Y, HLSProvider as a, VimeoProvider as b };
